...
getPartialQwp1(VFs, Phi1s, SPQs1):-
    ...
    %findall(PQ1, wp1(VFs,Phi1s,PQ1), PQs1U), !,
    garbage_collect,
    !.
...
getPartialQwp2(VFs, Phi1s, SPQs2):-
    ...
    %findall(PQ2, wp2(VFs,Phi1s,PQ2), PQs2U), !,
    garbage_collect,
    !.
...
wpi(Head, Prob, Act, Body, Phi1s, VFValue, VFState,
    ...
    write("in wpi: "), writeln(actionrule(Head, Prob, Act, Body, VFValue, VFState)),
    % get a subH
    subsetgen(Head, SubH),
    % get SubVFState
    ...
    write("SubH and VFSTail :   "), writeln([SubHp, VFSTail]),
    headbody(Head, VFValue, VFSTail, Prob, Act, Body, Phi1s,
             partialQ(Q,A,S,Size,Str,Sta)),
    write("ok   "), writeln(partialQ(Q,A,S,Size,Str,Sta)),nl.
